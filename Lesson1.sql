...
SELECT id, account_id, total_amt_usd 
FROM orders ORDER BY total_amt_usd
LIMIT 20;
...
SELECT id, account_id, total_amt_usd 
FROM orders ORDER BY total_amt_usd DESC, account_id
...
SELECT id, account_id, (poster_amt_usd/total_amt_usd)/100 AS poster_rev
FROM orders
LIMIT 10;
...
SELECT * FROM orders 
WHERE standard_qty > 1000 AND poster_qty
= 0 AND gloss_qty = 0;
...
SELECT * FROM web_events 
WHERE channel IN ('organic', 'adwords')
AND occurred_at BETWEEN '01-01-2016' AND '01-01-2017'
...
AND (gloss_qty > 1000 AND poster_qty > 1000);
...
AND primary_poc LIKE '%ana%' OR primary_poc
LIKE '%Ana%' AND primary_poc NOT LIKE '%eana%'
